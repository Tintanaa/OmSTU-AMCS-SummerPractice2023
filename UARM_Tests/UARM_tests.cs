...
namespace SpaceBattleTests
{
    ...
    public class »гровойќбъектћожетѕеремещатьс€ѕоѕр€мой
    {
        ...
        public void ѕроисходитѕр€молинейное–авномерноеƒвижениеЅезƒеформации()
        {
            try
            {
                _result.Add(ShipMovement.Shipmoving(_start, _finish, _isAllowToMove, _IsPosition, _IsSpeed));
            }
            catch { }
        }
        ...
        {
            Assert.Throws<Exception>(() => ShipMovement.Shipmoving(_start, _finish, _isAllowToMove, _IsPosition, _IsSpeed));
        }
    }
}